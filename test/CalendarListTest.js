...
describe('CalendarList.js', function () {
	...
	it('Should return http response body when CalendarList.get', () => {
		...
		let calendarListInstance = new calendarList(mockHttpRequest, 'jwt', 'gcalurl');
		return calendarListInstance.get(calendarId)
			.then((results) => {
				expect(expectedResult).to.eql(results);
			});
	});
	...
	it('Should return http response body when CalendarList.insert', () => {
		let calendarToInsert = 'calendarid@group.calendar.google.com';
		let inputParams = {
		...
		let calendarListInstance = new calendarList(mockHttpRequest, 'jwt', 'gcalurl');
		return calendarListInstance.insert(calendarToInsert, inputParams)
			.then((results) => {
				expect(expectedResult).to.eql(results);
			});
	});
	...
	it('Should return statusCode 204 & delete success message when calendarList.delete', () => {
		...
		let expectedResult = { calendarId: calendarToDelete, statusCode: mockResponse.statusCode, message: 'Calendar entry deleted successfully from CalendarList' };
